...
datagenA <- function(nsnps,snpsc,ss,beta1,beta2,beta3){
  ...
  G3 <- matrix(rbinom(n*snpsc, 2, af), n, snpsc)
  v_x1 <- rnorm(n,0,1)
  v_x2 <- rnorm(n,0,1.5)
  v_x3 <- rnorm(n,0,1.5)
...
datagenB <- function(nsnps,ss,beta1,beta2){
...
# ##using the unviariable MR snps

  mvmr_X1dat <- summary(lm(Y_b~-1+X1_b+X2_b,weights=1/(Y_se^2), data = MR_dat.X))

  res["mvad.X1_effect"] <- mvmr_X1dat$coefficients["X1_b","Estimate"]
  res["mvad.X1_se"] <- mvmr_X1dat$coefficients["X1_b","Std. Error"]
  res["mvad.X2_effect"] <-mvmr_X1dat$coefficients["X2_b","Estimate"]
  res["mvad.X2_se"] <- mvmr_X1dat$coefficients["X2_b","Std. Error"]
