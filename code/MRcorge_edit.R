##25/1/25
#This is the Code for MR corge taken from the R package. https://github.com/zhwm/MRCorge/tree/main
#credit: Zhang et al. Bioinformatics 2024

#Minor edits made so it will run with eaf missing and to the plots to edit the labels/titles. 
#otherwise no changes made from the package
...
#' @export
mrcorge_ed <- function(harmonized_data, rank = 'beta', K = 5, method_list = c("mr_ivw","mr_weighted_mode","mr_weighted_median"), seed = 1) {
  set.seed(seed)
  library(ggplot2)
  library(TwoSampleMR)
  harmonized_data <- subset(harmonized_data, mr_keep & !is.na(se.exposure))
